...
export const OutlinedButton = ({
  ...
  <button
    className={`inline-flex items-center py-1.5 mt-2 ${
      paddingWide ? "px-4" : "px-1"
    } bg-white text-${color}-400 rounded hover:shadow outline focus:outline-none ease-linear transition-all duration-150 hover:bg-${color}-100 tracking-wide font-inter ${
      borderVisible ? "border-2" : "border"
    } border-${color}-200 space-x-2`}
    type="button"
    onClick={() => action()}
  >
    {icon}
