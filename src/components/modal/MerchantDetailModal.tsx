    ...
    >
      {value}
        ...
        <textarea
            placeholder={value}
            readOnly={!editable}
            className={`border-none outline-none m-0 p-0 placeholder-gray-500 focus:ring-0 resize-none overflow-visible scrollbar-none ${editable ? "cursor-text" : "cursor-default"
                }`}
            title={editable ? "This can be edited" : ""}
            onChange={onChange}
        />
...
const MerchantModal: FC<ModalProps> = ({ showModal, action, merchant }) => {
  ...
  const validate = async () => {
    ...
    if (formData.contactPersonDesignation === '') {
      data.contactPersonDesignation = merchant?.merchant?.contactPersonDesignation
    } else {
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignContent: 'space-between' }}>
                                ...
                                <div className="grid grid-cols-4 gap-4 mb-4">
                                    ...
                                    <UserInfo
                                        ...
                                        onChange={(e: any) => setFormData({ ...formData, contactPersonPhone: e.target.value })}
                                    />
                                    <UserInfo
                                        editable={edit}
                                        title="Contact Person Email"
                                        value={merchant?.merchant?.contactPersonEmail}
                                        onChange={(e: any) => setFormData({ ...formData, contactPersonEmail: e.target.value })}
                                    />
                                    <UserInfo
                                        editable={edit}
                                        title="Contact Person Designation"
                                        value={merchant?.merchant?.contactPersonDesignation}
                                        onChange={(e: any) => setFormData({ ...formData, contactPersonDesignation: e.target.value })}
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-4 gap-4 mb-4">
                                    <UserInfo
                                        ...
                                        onChange={(e: any) => setFormData({ ...formData, account_issuer: e.target.value })}
                                    />
                                    <UserInfo
                                        editable={edit}
                                        title="Account Issuer Name"
                                        value={merchant?.merchant?.account_issuer_name}
                                        onChange={(e: any) => setFormData({ ...formData, account_issuer_name: e.target.value })}
                                    />
                                    <UserInfo
                                        editable={edit}
