...
type ModalProps={
 ...
 export const TransactionInfo=({title,value}:UserInfoProps)=>(
  ...
  const Modal=({showModal,transaction,approve,decline,cancel}:ModalProps)=>(
    <>
    { showModal &&
     (
            ...
            <div className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:min-w-sm w-2/3">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <TransactionInfo title="Date Initiated" value={formatDate(transaction.createdAt)}/>
                  <TransactionInfo title="Description" value={transaction?.description}/>
                  <TransactionInfo title="Amount" value={transaction?.amount}/>
                  <TransactionInfo title="Recipient Account Type" value={transaction?.recipient_account_type}/>
                  <TransactionInfo title="Recipient Account Name" value={transaction?.recipient_account_name}/>
                  <TransactionInfo title="Recipient Account Number" value={transaction?.recipient_account_number}/>
                  <TransactionInfo title="Payment Account Name" value={transaction?.payment_account_name}/>
                </div>
              </div>
