...
export default function Navbar() {
    ...
    return (
        <>
          <SlideDrawer drawerOpen={drawerOpen} close ={close} name={user?.name} email={user?.email} acc_type={user?.account_type} signout={signout} user={user}/>
                    ...
                    <ul className="flex-col md:flex-row list-none items-center hidden md:flex">  
                     {user&&<>
                      <span className='font-light'>active</span>
                       ...
                       {user!==null? <button className="text-red-700 uppercase font-bold bg-red-100 p-2 w-10 rounded-full" onClick={()=>setDrawerOpen(true)}>{acronym || 'N/A'}</button>
                       :
                        <button className="bg-pink text-white px-4 py-2 font-bold rounded" onClick={()=>navigate('/login')}>Login</button>
                       }
                    </ul>
                </div>
            </nav> 
