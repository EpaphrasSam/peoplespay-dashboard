    ...
    wallets.map((t:any)=>(
        ...
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-left">
            <span
                ...
                <span className="relative">
                    {`₵${Number(t?.totalBalance-t?.balance).toFixed(2)}`}
                </span>
            </span>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-left">
        ...
        <td className="text-left px-2 py-2 border-b border-gray-200 bg-white">
                <p className="whitespace-no-wrap">
                   <button onClick={()=>{goTo(t.customerId?._id||t.merchantId?._id,`${t&&t.customerId?.fullname || t&&t.merchantId.merchant_tradeName}`)}} className="text-md font-segoe text-blue-500 tracking-tight py-2 px-2 hover:underline">Details</button> 
                </p>
        </td>
    </tr>
    ))
