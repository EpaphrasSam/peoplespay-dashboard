...
import { merchantsSelector,setApps,setMerchantName } from '../../../state/merchant.state';
...
function MerchantsConfig(){
    ...
    const [merchant,setMerchant]=useState<any[]>([])
     ...
     const blockMerchant=(id:string,blocked:boolean)=>{
       try{
         confirmAlert({
            text:blocked?'This will unblock this merchant':'This will block this merchant',
            confirmButtonText:blocked?'Yes, unblock merchant':'Yes, block merchant'
         }).then(async(result)=>{
            ...
            <div className="inline-block min-w-full shadow-lg overflow-hidden">
                ...
                <div className="px-5 py-5 bg-white border-t flex flex-col sm:flex-row items-center sm:justify-between">
                    <span className="text-sm sm:text-sm text-gray-900">
                        Showing <span>{currentIndex * rowsPerPage - 10}{' '}</span> to{' '}<span>{currentIndex * rowsPerPage}</span> of <span>{approvedMerchants.length}</span>{' '}Entries
                    </span>
