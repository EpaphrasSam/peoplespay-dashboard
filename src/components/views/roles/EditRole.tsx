...
function EditRole() {
    ...
    switch (_access.title) {
      ...
      case "Merchants":
        _access.permissions.map((p: string) => {
          switch (p) {
            case "read":
              checkedList.current.push("30r");
              break;
            case "write":
              checkedList.current.push("30w");
              break;
            case "delete":
              checkedList.current.push("30d");
              break;
          }
        });
        break;
      case "Customers":
        _access.permissions.map((p: string) => {
          switch (p) {
            case "read":
              checkedList.current.push("31r");
              break;
            case "write":
              checkedList.current.push("31w");
              break;
            case "delete":
              checkedList.current.push("31d");
              break;
          }
        });
        break;
  ...
  const accessClick = (e: any) => {
    const isExist: boolean = accessArray.some(
      (obj) => obj.title === e.target.name
    );
    if (isExist) {
      ...
      checkedList.current
        ...
        .forEach((id) =>
          checkedList.current.splice(checkedList.current.indexOf(id), 1)
        );
      ...
      accessArray?.map((obj) => {
        if (obj?.title === e.target.name) {
          ...
          if (isExist) {
            ...
            return {
              ...obj,
              permissions: [...obj.permissions, e.target.value],
            };
          }
        } else return obj;
      })
    ...
    try {
      ...
      await alertResponse({
        ...
        response: res.success
          ? res.message
          : "Invalid form data. Please provide the required data",
      });
      if (res.success) {
  ...
  return (
    <div className="relative md:pt-10 pb-10 p-2">
      ...
      <div className="w-full">
        <AccessTable
          accessClick={(e: any) => accessClick(e)}
          permissionClick={(e: any) => permissionClick(e)}
          checkedList={checkedList.current}
